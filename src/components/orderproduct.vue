<template>
  ...
  <div style="position:absolute ; text-align:center;padding:10px; width:30% ; top:85px ;height:600px; background:white ; left: 10px">
    ...
    <b-input style="text-align: center" readonly :value="$route.params.id"> 1</b-input>
    <label for="">محصول</label>
    <b-input style="text-align: center" readonly :value="model.parts[0].itemname"> 1</b-input><br>
    <table v-if="final.length" class="table table-dark">
      <tr>
        <th>پارچه</th>
        <th>بخش</th>
      </tr>
      <tr  v-for="(item, idx) in model.parts" v-bind:key="idx">
        <td>{{final[idx]}}</td>
        <td>{{item.name}}</td>
      </tr>
    </table>
...
export default {
  ...
  methods: {
    async getitemmaterials () {
      await axios
        .get(`/itemmaterials2/${this.$route.params.id}`)
        .then(response => {
    ...
    loadm(counter){
            ...
            this.obj.push(object)
              if(this.model.parts[counter].default.code){
                this.setcolor(counter , this.model.parts[counter].default.code , this.model.parts[counter].default.shininess ,  this.model.parts[counter].default.repeat ,  this.model.parts[counter].default.normal , this.model.parts[counter].default.id ,this.model.parts[counter].default.name  )
              }else{
                this.setcolor2(counter , this.model.parts[counter].default.pic , this.model.parts[counter].default.shininess , this.model.parts[counter].default.repeat ,  this.model.parts[counter].default.normal , this.model.parts[counter].default.id ,this.model.parts[counter].default.name  )
              }
    ...
    setcolor(counter , code , shininess ,repeat , normal , id , name) {
    ...
    setcolor2(counter , code , shininess , repeat , normal , id , name) {
      this.final.push(name)
      this.$loading(true)
      if(!this.bump){
    ...
    setcolor3(counter ,pic, code , shininess, repeat , normal , id , name) {
      this.final[counter] = name
      this.$loading(true)
      if(!this.bump){
        this.bump = this.model.parts[counter].bump[0].pic
      }
