  ...
  methods: {
    ...
    async getmat () {
      await axios
        ...
        .then(response => {
          var mat = response.data[0]
          console.log(mat)
          this.repeat = mat.repeat
          this.normal = mat.bumpmap
          this.name = mat.name
          this.getitem()
        })
    },
    async submit(){
      ...
      await axios
        .put(`/material/${this.$route.params.id}`, formdata)
        .then(response => {
    ...
    setcolor() {
      ...
      if (normal !== null){
        var bu = new THREE.TextureLoader().load( normal )
          ...
          var material = new THREE.MeshPhongMaterial( {normalMap: bu , color: Number(code) , shininess : shininess} );
      }else{
          var material = new THREE.MeshPhongMaterial( { color: Number(code) , shininess : shininess} );
      }      
